...
st.set_page_config(page_title="Likelihood-Free Amortized Inference", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🧠 Likelihood-Free Amortized Inference</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Neural posterior sampling | No MCMC | Instantaneous inference for rolling windows</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 LFI")
...
st.sidebar.markdown(f"**Hidden size:** {config.HIDDEN_SIZE} | **Latent dim:** {config.LATENT_DIM}")
st.sidebar.markdown(f"**Seq len:** {config.SEQ_LEN} | **Samples:** {config.N_POSTERIOR_SAMPLES}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'lfi_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Posterior mean: {etf['lfi_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Posterior Mean"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Posterior Mean", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🧠 Top ETFs by Posterior Mean (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Likelihood-free amortized inference** trains a neural network to directly output posterior samples.
        - Once trained, inference is **instantaneous** – no MCMC required.
        - Ideal for online settings where models must be repeatedly fit to rolling windows.
        - The score is the **posterior mean** of the expected return.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the posterior means.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Likelihood-Free Amortized Inference | Neural posterior sampling for rolling windows")
